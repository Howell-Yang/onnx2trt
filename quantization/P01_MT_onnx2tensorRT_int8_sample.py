...
MODEL = "Models/RMTNet_release20220609_v2.opt.onnx"
...
if PPQ_CONFIG.USING_CUDA_KERNEL:
    ...
    QS.lsq_optimization_setting.epochs = 4  # 再训练轮数，影响训练时间，30轮大概几分钟
...
with ENABLE_CUDA_KERNEL():
    ...
    export_ppq_graph(
        graph=qir,
        platform=QUANT_PLATFROM,
        graph_save_to=MODEL.replace(".onnx", "_int8_sample2.onnx"),
    )
    ...
    with open(MODEL.replace(".onnx", "_int8_sample2.engine"), "rb") as f, trt.Runtime(
        logger
    ) as runtime:
