...
class OnnxOptimizer:
    def __init__(self, onnx_model_path=sys.argv[1], calibration_data=input_data):
        ...
        # 移除input中的initializers
        # 将constants移到initilizers中
    ...
    def input_no_quantization_tensors(self, onnx_model):
        ...
        for node in onnx_model.graph.node:
            ...
            # 某些节点输出不量化
            elif node.op_type in ["Reshape", "Transpose", "MaxPool", "InstanceNormalization"]:
                ret_tensors.append(node.output[0])
    ...
    def remove_initializer_from_input(self, model):
        ...
        for node in constant_nodes:
            ...
            elif attr.type == onnx.AttributeProto.AttributeType.STRING or \
                attr.type == onnx.AttributeProto.AttributeType.STRINGS:
                print(node.name, "attr type not float or int", attr.type)
                raise NameError("Constant to Initializer not Supported: String or Strings")
    ...
    def process(self):
        ...
        while len(self.next_to_quant) > 0:
            ...
            while len(post_nodes) > 0:
                ...
                # 单输入，无权重，单输出，向后传递的节点
                elif post_node.op_type in ["Reshape", "Transpose", "MaxPool", "Resize", "InstanceNormalization"]:
                    if (
                        post_node.output[0] in self.no_need_to_quantize
                    ):  # Conv-Relu/Conv-Concat的传递
                        self.already_quantized.append(post_node.output[0])
