...
class OnnxOptimizer:
    def __init__(self, onnx_model_path=sys.argv[1], calibration_data=input_data):
        ...
        self.already_quantized += self.constant_names  # 所有的固定值不做量化(权重不是Constant)
    ...
    def process(self):
        ...
        while len(self.next_to_quant) > 0:
            ...
            while len(post_nodes) > 0:
                ...
                # 单输入，无权重，单输出，向后传递的节点
                elif post_node.op_type in ["Reshape", "Transpose", "Resize", "MaxPool"]:
                    if (
                        post_node.output[0] in self.no_need_to_quantize
                    ):  # Conv-Relu/Conv-Concat的传递
                        self.already_quantized.append(post_node.output[0])
                ...
                print("已量化的节点:", self.already_quantized[-20:])
                print("准备好量化的节点:", self.next_to_quant)
