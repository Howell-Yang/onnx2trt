...
MODEL = "model_copy_adaround.onnx"
...
# 下面向你展示了常用参数调节选项：
# -------------------------------------------------------------------
if True:  # PPQ_CONFIG.USING_CUDA_KERNEL:
...
with ENABLE_CUDA_KERNEL():
    ...
    export_ppq_graph(
        graph=qir,
        platform=QUANT_PLATFROM,
        graph_save_to="model_copy_adaround_int8.onnx",
    )
    ...
    with open("model_copy_adaround_int8.engine", "rb") as f, trt.Runtime(
        logger
    ) as runtime:
    ...
    with engine.create_execution_context() as context:
        inputs, outputs, bindings, stream, output_names = trt_infer.allocate_buffers(
            context.engine
        )
        for sample in tqdm(samples, desc="TensorRT is running..."):
            inputs[0].host = convert_any_to_numpy(sample)
            outputs_list = trt_infer.do_inference(
                context,
                bindings=bindings,
                inputs=inputs,
                outputs=outputs,
            ...
            for output in outputs_list:
                print(np.reshape(output, (1, -1))[0, :10])
