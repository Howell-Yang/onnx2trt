...
MODEL          = 'model_lsq.onnx'
INPUT_SHAPE    = [1, 3, 384, 768]

calibration_files = glob.glob(os.path.join("/apdcephfs/private_howellyang/data/Calib1k5/", '*.jpg'))[:256]
SAMPLES        = [read_image(path) for path in calibration_files] # rewirte this to use real data.
...
if PPQ_CONFIG.USING_CUDA_KERNEL:
    ...
    QS.lsq_optimization_setting.epochs = 64                         # 再训练轮数，影响训练时间，30轮大概几分钟
    QS.lsq_optimization_setting.collecting_device = 'cuda'          # 缓存数据放在那，cuda 就是放在gpu，如果显存超了你就换成 'cpu'

QS.dispatching_table.append(operation='Sigmoid', platform=TargetPlatform.FP32) # 把量化的不太好的算子送回 FP32
...
with ENABLE_CUDA_KERNEL():
    ...
    export_ppq_graph(
        ...
        graph_save_to = 'model_copy_int8.onnx')
    ...
    with open('model_copy_int8.engine', 'rb') as f, trt.Runtime(logger) as runtime:
